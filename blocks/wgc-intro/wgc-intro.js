 ...
 * decorate the block
 *
 * Authors supply one media cell and one copy cell, in either order. The cells
 * are classified in place rather than rebuilt, so authoring instrumentation and
...
export default function decorate(block) {
  getRows(block).forEach((row) => {
    getCells(row).forEach((cell) => {
      ...
      cell.classList.add(isMedia ? 'wgc-intro-media' : 'wgc-intro-copy');
    });
  });
  ...
  block.querySelectorAll('.wgc-intro-media picture > img').forEach((img) => {
