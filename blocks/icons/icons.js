...
export default function decorate(block) {
  ...
  getRows(block).forEach((row) => {
    ...
    getCells(row).forEach((cell) => {
      ...
      cell.classList.add(isIcon ? 'icons-icon' : 'icons-label');
      li.append(cell);
    });
    ...
    li.querySelectorAll('.icons-label a.button').forEach((a) => {
  ...
  ul.querySelectorAll('.icons-icon picture > img').forEach((img) => {
