...
export default function decorate(block) {
  ...
  getRows(block).forEach((row) => {
    ...
    getCells(row).forEach((cell) => {
      ...
      cell.classList.add(isIcon ? 'wgc-icons-icon' : 'wgc-icons-label');
      li.append(cell);
    });
    ...
    li.querySelectorAll('.wgc-icons-label a.button').forEach((a) => {
  ...
  ul.querySelectorAll('.wgc-icons-icon picture > img').forEach((img) => {
