 ...
 * can play, so an authored video never delays first paint.
 ...
 * decorate the block
 *
 * Two authored rows: a background image (with an optional video link) and the
 * breadcrumb label plus page heading.
 * @param {Element} block the block
 */
export default function decorate(block) {
  const media = document.createElement('div');
  media.className = 'wgc-meetings-hero-media';
  const content = document.createElement('div');
  content.className = 'wgc-meetings-hero-content';
  ...
  markEyebrow(content, heading, 'wgc-meetings-hero-eyebrow');
  if (heading) {
    splitHeading(heading);
    addRule(heading, { centered: !block.classList.contains('left'), light: true });
  }

  // the banner image is above the fold, so it loads eagerly as the LCP candidate
